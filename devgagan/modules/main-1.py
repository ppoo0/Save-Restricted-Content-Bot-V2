...
async def single_link(_, message):
    ...
    # ✅ FIX 2: Check users_loop properly — treat missing key as False
    ...
    try:
        ...
        # ✅ FIX 2: Always clear users_loop in finally so it never stays stuck
...
async def batch_link(_, message):
    ...
    # ✅ FIX 2: Check users_loop properly
    if users_loop.get(user_id, False):
    ...
    try:
        ...
        # ✅ FIX 1: Determine link type ONCE before the loop
        # Check the first link to decide which loop to run
        first_url = f"{'/'.join(start_id.split('/')[:-1])}/{cs}"
        first_link = get_link(first_url)
        is_special = any(x in first_link for x in ['t.me/b/', 't.me/c/'])

        if is_special:
            # Special links (private channels) — need userbot
            if not userbot:
                await app.send_message(message.chat.id, "Login in bot first to process private channel links.")
                return

            for i in range(cs, cs + cl):
                # ✅ FIX 2: Check cancellation flag each iteration
                if not users_loop.get(user_id, False):
                    await app.send_message(message.chat.id, "Batch cancelled.")
                    return

                url = f"{'/'.join(start_id.split('/')[:-1])}/{i}"
                link = get_link(url)
                msg = await app.send_message(message.chat.id, f"Processing {i - cs + 1}/{cl}...")
                try:
                    await process_and_upload_link(userbot, user_id, msg.id, link, 0, message)
                except Exception as e:
                    await app.send_message(message.chat.id, f"⚠️ Error on file {i - cs + 1}: {e}")
            ...
            # Normal public links — no userbot needed
            for i in range(cs, cs + cl):
                # ✅ FIX 2: Check cancellation flag each iteration
                if not users_loop.get(user_id, False):
                    await app.send_message(message.chat.id, "Batch cancelled.")
        ...
        # ✅ FIX 2: Always clear in finally block — no matter what happens
